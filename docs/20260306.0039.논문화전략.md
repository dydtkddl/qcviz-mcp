아래에 기존 보고서 5개 섹션·19개 소제목 구조를 그대로 유지하면서, 이번 세션에서 축적된 Phase ζ 결과(전이금속·UHF·PNG·108 테스트 등)를 반영한 전체 업데이트 보고서를 출력합니다. 변경·추가된 문장 끝에 `← updated`를 표기했습니다.

---

# QCViz‑MCP 시스템 전체 보고서

**Version 0.5.0‑alpha · 2026‑03‑05 · Commit ad29790→2cf415f** ← updated
...
이 시스템은 "물 분자의 결합 오비탈을 보여줘" 같은 자연어 한 줄 입력으로, 기존에 Python 스크립트 20줄 이상이 필요했던 SCF→IAO→IBO→큐브→3D 렌더링 5단계 워크플로우를 1단계로 축약하며, 전이금속 ECP 분자와 개방껍질(UHF/ROHF) 시스템까지 단일 인터페이스로 처리한다. ← updated
...
6개 도구 모두 기능 구현이 완료되어 MCP 프로토콜을 통해 호출 가능하다. ← updated (이전 보고서에서 2/6 논문 수준, 1/6 업그레이드, 3/6 부분 구현이었으나, Phase ε에서 나머지 3개가 기능 완료됨)

### 2.3 백엔드 상세

**PySCFBackend** – 핵심 계산 엔진. RHF/UHF/ROHF SCF 수행, IAO 투영, IBO 로컬라이제이션(Pipek‑Mezey), 큐브 파일 생성, Molden 내보내기를 담당한다. ECP(Effective Core Potential) 분자에 대한 MINAO 폴백 로직(`_resolve_minao()`)과 UHF α/β 오비탈 분리 처리(`_unpack_uhf()`, `compute_scf_flexible()`, `compute_iao_uhf()`, `compute_ibo_uhf()`, `compute_uhf_charges()`)를 포함한다. ← updated
...
8. 선택적으로 헤드리스 PNG 렌더러(Playwright + Chromium + SwiftShader)를 통해 정적 이미지를 내보낼 수 있다. ← updated
9. 결과가 JSON 응답으로 MCP 클라이언트에 반환된다.

전체 실행 시간은 water/STO‑3G에서 약 0.1초, ethylene/cc‑pVDZ에서 약 0.8초이다.

개방껍질 분자(예: O₂ 삼중항, NO 라디칼)의 경우, `compute_scf_flexible(spin=N)` → UHF SCF → `compute_iao_uhf()` → `compute_ibo_uhf()`로 α/β 채널이 분리 처리되며, 동일한 MCP 인터페이스를 통해 호출된다. ← updated

### 2.5 정량적 지표

| 지표               | 값                                                                                          |
| ------------------ | ------------------------------------------------------------------------------------------- |
| 테스트 통과        | **108 passed** ← updated                                                                    |
| 테스트 실패        | **0 failed** ← updated                                                                      |
| 테스트 스킵        | **4 skipped** (Playwright 미설치 시 PNG 테스트) ← updated                                   |
| 전체 실행 시간     | **13.83 s** ← updated                                                                       |
| 벤치마크 분자 수   | **13** (기존 10 NIST + Fe(CO)₅ + TiCl₄ + NO) ← updated                                      |
| 벤치마크 기저 함수 | STO‑3G, cc‑pVDZ (폐각 10분자), def2‑SVP (전이금속 2분자) ← updated                          |
| 벤치마크 성공률    | **20/20** (폐각 10분자 × 2기저) + 전이금속·개방껍질 개별 검증 ← updated                     |
...
| 원소 범위          | H–Fe (Z = 1–26) ← updated                                                                   |
| 스핀 처리          | RHF + UHF (ROHF 경유 가능) ← updated                                                        |
| 이미지 출력        | HTML (인터랙티브) + PNG (헤드리스, 선택적) ← updated                                        |

### 2.6 의존성 및 환경

**런타임 의존성:** Python ≥ 3.10, PySCF (conda‑forge, SCF/IAO/IBO/cube/Molden), cclib 1.8.1 (출력 파싱), py3Dmol 2026‑01‑22 (3D 시각화), ASE (구조 변환), FastMCP ≥ 3.0.2 (MCP 서버 프레임워크).

**선택적 의존성:** Playwright ≥ 1.40 (헤드리스 PNG 내보내기, `pip install qcviz-mcp[png]`로 설치). ← updated
...
**지원 플랫폼:** Linux/WSL2 (전체 기능), Windows (PySCF 미지원 시 `dev-no-pyscf` 모드, 31 passed / 16 skipped), macOS (미검증). ← updated
...
QCViz‑MCP는 IAO/IBO 로컬라이제이션을 MCP 프로토콜로 제공하는 최초이자 유일한 서버이다. 2026년 3월 전수조사(~20개 프로젝트 검토) 결과, MCP 생태계 내에 전자구조 계산을 수행하는 서버는 존재하지 않았다. 가장 근접한 Argonne mcp.science는 논문 검색과 데이터 쿼리에 국한되며, 오비탈 분석 기능이 없다. Phase ζ에서 전이금속(Fe, Ti) ECP 지원과 개방껍질(UHF) 처리가 추가되어, 커버 가능한 화학 공간이 유기 분자(H–N)에서 전이금속 착물(H–Fe)과 라디칼까지 확장되었다. ← updated
...
UHF 경로에서도 α/β 채널 각각에 대해 IAO/IBO 수와 전하 합이 검증되며, 전이금속 ECP 분자에서는 MINAO 폴백이 자동 적용되고 수렴 여부가 확인된다. ← updated
...
| 자체 품질 검증         |    ✅     |    ❌    |     ❌      |    ❌     |   ❌    |      ❌       |
| 전이금속 ECP           |    ✅     |    ❌    |     ❌      |    ❌     |   ✅    |      ✅       | ← updated |
| UHF/ROHF 개방껍질      |    ✅     |    ❌    |     ❌      |    ❌     |   ✅    |      ✅       | ← updated |
| 헤드리스 PNG 내보내기  |    ✅     |    ❌    |     ❌      |    ❌     |   ❌    |      ❌       | ← updated |

QCViz‑MCP는 위 10개 항목 모두를 충족하는 유일한 도구이다. ← updated
...
108개 테스트(자동화), 20개 벤치마크(NIST 표준 분자), 13개 벤치마크 분자(폐각 10 + 전이금속 2 + 라디칼 1), 6개 MCP 도구, 4개 백엔드, IBO 검증 파이프라인(4개 메트릭), Molden 내보내기, UHF α/β 분리 처리, ECP MINAO 폴백, MkDocs API 문서(5페이지), 헤드리스 PNG 렌더링 – 이 모든 구성 요소가 단일 저장소에 통합되어 있다. ← updated
...
JOSS 논문에서 화학 영역 MCP 서버의 부재, IBO의 학술적 중요성(801+ 인용), 기존 도구(IboView, Multiwfn, Jmol)의 MCP 미지원·자동화 한계를 서술한다. 전이금속과 개방껍질 시스템에 대한 자동화된 IBO 분석 도구가 기존에 없었음을 강조한다. ← updated

### 4.4 소프트웨어 설계 (Software Design)

FastMCP 3.x 선택 이유(async‑native, SSE 지원, Python 타입 힌트 기반 스키마 자동 생성), BackendRegistry 플러그인 패턴(확장성), 보안 설계 3가지(경로 순회 차단 `_validate_file_path`, 원자 수 제한 `_validate_atom_spec`, 입력 크기 제한), UHF 3D ndarray 호환 레이어(`_unpack_uhf`)를 논문에 포함한다. ← updated

### 4.5 연구 영향 (Research Impact Statement)

JOSS 2026년 1월 정책 업데이트에 따라 연구 영향 서술이 강화되었다. QCViz‑MCP의 영향은 다음과 같이 정량화된다:

- 10개 NIST 표준 분자 × 2개 기저 함수 벤치마크: 20 케이스 전수 통과, 에너지·IBO 수·전하 보존 모두 검증.
- 전이금속 분자(Fe(CO)₅, TiCl₄) ECP 자동 폴백 및 수렴 검증. ← updated
- 개방껍질 분자(O₂ 삼중항, NO 라디칼) UHF α/β IBO 분리 계산 성공. ← updated
- 기존 5단계 워크플로우 → 1단계 자연어 축약: 코드 35줄 → 0줄.
- 헤드리스 PNG 내보내기를 통한 논문 Figure 자동 생성 가능. ← updated

### 4.6 AI 사용 고지 (AI Usage Disclosure)

JOSS 2026년 1월 정책 업데이트에 따라 AI 도구 사용을 투명하게 공개한다:

- **사용 도구:** Anthropic Claude (Claude Opus 4 계열) ← updated
- **사용 범위:** 코드 스캐폴딩(boilerplate 생성), 테스트 코드 초안 작성, 문서 초안 작성, 디버깅 제안.
- **인간 기여:** 전체 아키텍처 설계, 핵심 알고리즘 로직(IAO/IBO 파이프라인, ECP 폴백 로직, UHF 분리 처리), 모든 코드 리뷰 및 수정, 벤치마크 설계 및 결과 검증, 논문 내러티브 구성. ← updated
- **검증:** 모든 AI 생성 코드는 인간이 리뷰하고, 108개 자동화 테스트와 20개 벤치마크로 검증되었다. ← updated

### 4.7 JOSS 준수 체크리스트

| JOSS 기준       | 증거                                                                     | 상태         |
| --------------- | ------------------------------------------------------------------------ | ------------ |
| 학술 노력       | 108 테스트, 20 벤치마크, 6 도구, 4 백엔드, 검증 파이프라인, UHF/ECP 지원 | ✅ ← updated |
| 연구 목적       | 교육·연구 접근 장벽 제거, 전이금속·라디칼 지원                           | ✅ ← updated |
| 분야 현황       | 전수조사(~20개 프로젝트), MCP 생태계 내 유일한 전자구조 서버             | ✅           |
| 소프트웨어 설계 | 3‑계층, FastMCP, BackendRegistry, 보안 3중 검증, UHF 호환 레이어         | ✅ ← updated |
| 연구 영향       | 10 NIST 분자 벤치마크 100%, 전이금속·개방껍질 검증, PNG 내보내기         | ✅ ← updated |
...
QCViz‑MCP는 PySCF의 IAO/IBO 계산을 FastMCP 3.x 기반 MCP 서버로 래핑하여, 자연어 한 줄 입력으로 SCF→IAO→IBO→큐브→3D 시각화 전체 파이프라인을 실행한다. 3‑계층 아키텍처, BackendRegistry 플러그인, 보안 3중 검증, ECP 자동 폴백, UHF α/β 분리 처리를 포함한다. ← updated

**제3막: Evidence (Research Impact + Benchmarks)**
10개 NIST 표준 분자 × 2개 기저 함수 벤치마크 100% 통과. 108개 자동화 테스트 0 실패. IBO 품질 검증(spread, gradient, round‑trip, charge conservation) 전수 통과. 전이금속(Fe(CO)₅, TiCl₄) ECP 수렴 확인. 개방껍질(O₂, NO) UHF IBO 검증 완료. ← updated
...
| State of the Field  | 경쟁 도구 비교(ChemLint, mcp.science, IboView), 차별점       | 150–200 단어           |
| Software Design     | 3‑계층 아키텍처, FastMCP, BackendRegistry, 보안, UHF/ECP     | 200–300 단어 ← updated |
| Research Impact     | 벤치마크 수치, 워크플로우 축약, 전이금속·개방껍질 검증       | 150–200 단어 ← updated |
| AI Usage Disclosure | Claude 사용 범위·검증·인간 기여                              | 100–150 단어           |
| **합계**            |                                                              | **950–1,300 단어**     |

목표 단어 수는 JOSS 제한(750–1,750) 내에서 1,100–1,300 단어로 설정한다.

### 5.3 Figure 및 Table 계획

| #       | 유형                | 내용                                                          | 형식               |
| ------- | ------------------- | ------------------------------------------------------------- | ------------------ |
| Fig. 1  | 아키텍처 다이어그램 | 3‑계층 (MCP 프로토콜 → 6 도구 → 4 백엔드) + UHF/ECP 경로 표시 | SVG/PDF ← updated  |
| Fig. 2  | 워크플로우 비교     | 기존 5단계 스크립트 vs. QCViz‑MCP 1단계 자연어                | SVG/PDF            |
| Fig. 3  | IBO 시각화 예시     | 물 분자 σ/lone‑pair IBO 3D 렌더링 (PNG, 헤드리스 내보내기)    | PNG ← updated      |
| Table 1 | 벤치마크 결과       | 10(+3) 분자 × 기저 함수: 에너지, IBO 수, 실행 시간            | Markdown ← updated |
| Table 2 | 경쟁 도구 비교      | 10개 기능 항목 × 6개 도구 (섹션 3.4 표 확장)                  | Markdown ← updated |
...
- **전이금속 제한:** Fe(CO)₅, TiCl₄ 수준까지 검증되었으나, 란타나이드·악티나이드·무거운 전이금속은 미검증이다. ← updated
- **py3Dmol 유지보수:** py3Dmol의 최종 업데이트가 2026‑01‑22이며, 활발한 유지보수가 이루어지지 않고 있다. 장기적으로 대체 렌더러(3Dmol.js 직접 호출 등)를 고려해야 한다.
- **UHF/ROHF 실험적:** O₂ 삼중항과 NO 라디칼에서 검증되었으나, 대형 개방껍질 시스템(전이금속 착물의 고스핀 상태 등)에서의 수렴성은 추가 검증이 필요하다. ← updated
- **헤드리스 PNG:** Playwright + Chromium 의존성이 무겁고(~200 MB), CI 환경에서 추가 설정이 필요하다. ← updated
- **기저 함수 범위:** STO‑3G와 cc‑pVDZ(폐각), def2‑SVP(전이금속)만 벤치마크되었다. 더 큰 기저(cc‑pVTZ, aug‑cc‑pVDZ 등)에서의 성능은 미확인이다. ← updated

### 5.5 두 줄 요약과 내러티브의 정합성

본 보고서 제1절의 두 줄 요약("세계 유일의 전자구조 MCP 서버", "자연어 한 줄로 5단계 → 1단계 축약, 전이금속·개방껍질 포함")은 제5절의 4막 내러티브 결론("양자화학 접근 장벽을 자연어 수준으로 낮추고 MCP 생태계에 새 카테고리 개척, AI‑과학 통합의 참조 구현")과 일관된다. ← updated
...
| ε     | 2026‑03‑05 | parse_output, partial_charges, convert_format 구현, Molden 내보내기, IBO 검증 파이프라인, JOSS 6섹션 완성, 95 테스트, v0.4.0‑alpha | v0.4.0‑alpha |
| ζ     | 2026‑03‑05 | 전이금속 ECP 폴백, UHF α/β IBO, 헤드리스 PNG, 108 테스트, 13 벤치마크 분자, v0.5.0‑alpha                                           | v0.5.0‑alpha | ← updated |

---

_이 보고서는 `docs/SYSTEM_REPORT.md`로 저장되며, 프로젝트 내부 기록·대외 발표·JOSS 논문 초안의 기반 문서로 활용된다._

_최종 업데이트: 2026‑03‑05, v0.5.0‑alpha, 108 passed / 0 failed / 4 skipped_ ← updated
