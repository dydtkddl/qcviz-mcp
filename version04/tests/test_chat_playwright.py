...
def test_playwright_semantic_dropdown_uses_grounded_candidate_not_water(live_server_url):
    ...
    assert "water" not in all_text.lower()
...
def test_playwright_semantic_selection_completes_without_second_composition_prompt(live_server_url):
    ...
    with playwright.sync_playwright() as p:
        ...
        page.goto(live_server_url, wait_until="domcontentloaded")
        page.wait_for_selector("#chatInput")
        page.locator("#chatInput").fill("main component of TNT HOMO 보여줘")
        page.wait_for_function("() => { const btn = document.querySelector('#chatSend'); return btn && !btn.disabled; }")
        page.locator("#chatSend").click()
...
def test_playwright_question_like_acronym_routes_to_chat_without_clarify(live_server_url):
    ...
    with playwright.sync_playwright() as p:
        ...
        page.goto(live_server_url, wait_until="domcontentloaded")
        page.wait_for_selector("#chatInput")
        page.locator("#chatInput").fill("MEA알아?")
        page.wait_for_function("() => { const btn = document.querySelector('#chatSend'); return btn && !btn.disabled; }")
        page.locator("#chatSend").click()
        page.wait_for_function(
            "() => Array.from(document.querySelectorAll('.chat-msg')).some((el) => ((el.textContent || '').toLowerCase().includes('abbreviation')))"
        )
        page.wait_for_timeout(500)
        clarify_count = page.locator('.chat-msg--clarify').count()
        all_text = page.locator('#chatMessages').text_content() or ''
    ...
    assert "abbreviation" in all_text.lower()
