...
class QCVizAgent:
    ...
    def _plan_with_llm_preference(
        ...
        *,
        ...
        normalized = normalized or normalize_user_text(message)
        normalized_text = normalized.get("normalized_text") or message
        chosen = self._choose_provider()
        context = context or {}

        if chosen == "openai":
            return self._plan_with_openai(normalized_text, context=context)

        if chosen == "gemini":
            try:
                return self._plan_with_gemini_structured(message, normalized, context=context)
            except Exception:
                return self._plan_with_gemini(normalized_text, context=context)
        ...
        if chosen == "auto":
            if self.gemini_api_key:
                try:
                    ...
                    logger.debug("Gemini structured stage-3 planning failed; trying other providers.", exc_info=True)
            if self.openai_api_key:
                return self._plan_with_openai(normalized_text, context=context)
            if self.gemini_api_key:
                return self._plan_with_gemini(normalized_text, context=context)

        raise RuntimeError("No LLM provider available for stage-3 planning")
    ...
    def _heuristic_plan(
        ...
        early_follow_up_mode = self._none_if_blank(normalized.get("follow_up_mode"))
        early_selected = list(normalized.get("selected_molecules") or [])
        early_mentioned = list(normalized.get("mentioned_molecules") or [])
        early_batch_request = bool(normalized.get("batch_request")) or len(early_selected) > 1 or len(early_mentioned) > 1
        if early_batch_request:
            early_follow_up_mode = None

        if early_follow_up_mode and not normalized.get("maybe_structure_hint") and not normalized.get("structures"):
            query_kind = "compute_ready"
            semantic_grounding_needed = False

        if bool(normalized.get("chat_only")) and not early_follow_up_mode and not early_batch_request:
        ...
        if batch_request or len(mentioned_molecules) >= 2:
            ...
            follow_up["follow_up_mode"] = None
            follow_up["requires_context"] = False
        ...
        if follow_up.get("follow_up_mode") and not structure_query and not structures and not selected_molecules:
            query_kind = "compute_ready"
            semantic_grounding_needed = False

        if (batch_request or len(mentioned_molecules) >= 2) and not selected_molecules:
            selected_molecules = []
            for item in mentioned_molecules:
                ...
                if token and token.lower() not in {entry.lower() for entry in selected_molecules}:
                    selected_molecules.append(token)
            if not selected_molecules:
                selected_molecules = [
                    str(item).strip()
                    for item in (normalized.get("canonical_candidates") or [])
                    if str(item).strip()
                ]
            if selected_molecules:
                batch_request = len(selected_molecules) > 1
        ...
        if intent != "chat" and not structure_query and not structures and not batch_request and not selected_molecules:
    ...
    def _apply_structure_hypotheses(
        ...
        *,
        ...
        follow_up_mode = self._none_if_blank(out.get("follow_up_mode") or normalized.get("follow_up_mode"))
        intent = str(out.get("intent") or out.get("job_type") or "analyze").strip()
        is_computation_intent = intent not in {"chat"}

        if follow_up_mode and current_structure and self._is_task_like_structure_query(current_structure):
            out["structure_query"] = None
            current_structure = None
        if primary_candidate and self._is_task_like_structure_query(primary_candidate):
    ...
    def _coerce_plan(self, data: Dict[str, Any], provider: str) -> AgentPlan:
        ...
        if structure_analysis.get("canonical_candidates"):
            ...
            if structure_analysis.get("multi_molecule"):
                ...
                selected = list(data.get("selected_molecules") or [])
                if not selected:
                    for item in list(structure_analysis.get("mentioned_molecules") or []):
                        if isinstance(item, dict):
                            token = str(
                                item.get("canonical_name")
                                or item.get("alias")
                            ...
                            token = str(item).strip()
                        if token and token.lower() not in {entry.lower() for entry in selected}:
                            selected.append(token)
                if not selected:
                    selected = list(structure_analysis.get("canonical_candidates") or [])
                if not data.get("selected_molecules"):
                    data["selected_molecules"] = selected
                if data.get("batch_request") in (None, False):
                    data["batch_request"] = len(selected) > 1
                if not data.get("batch_size"):
                    data["batch_size"] = len(selected)
        ...
        if _value_missing(data.get("query_kind")):
            data["query_kind"] = routing.get("query_kind")
        if _value_missing(data.get("semantic_grounding_needed")):
            data["semantic_grounding_needed"] = routing.get("semantic_grounding_needed")
        if _value_missing(data.get("unknown_acronyms")):
            data["unknown_acronyms"] = list(routing.get("unknown_acronyms") or [])
        if _value_missing(data.get("mentioned_molecules")):
            data["mentioned_molecules"] = []
        if _value_missing(data.get("selected_molecules")):
            data["selected_molecules"] = []
        if _value_missing(data.get("analysis_bundle")):
        ...
        return AgentPlan(
            ...
            planner_lane=parsed.planner_lane,
            lane_locked=bool(parsed.lane_locked),
            locked_lane=parsed.locked_lane,
            semantic_grounding_needed=bool(parsed.semantic_grounding_needed),
            question_like=bool(parsed.question_like),
            explicit_compute_action=bool(parsed.explicit_compute_action),
            explanation_intent=bool(parsed.explanation_intent),
            grounding_intent=bool(parsed.grounding_intent),
            compute_intent=bool(parsed.compute_intent),
            ...
            provider=provider,
            pipeline_enabled=bool(parsed.pipeline_enabled),
            pipeline_fallback_stage=parsed.pipeline_fallback_stage,
            pipeline_repair_count=parsed.pipeline_repair_count,
