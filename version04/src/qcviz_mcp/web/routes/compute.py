...
from qcviz_mcp.llm.normalizer import (
    ...
    _looks_like_plain_molecule_name,
...
def _public_plan_dict(plan: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "planner_lane": out.get("planner_lane"),
        "lane_locked": out.get("lane_locked"),
        "locked_lane": out.get("locked_lane"),
        "semantic_grounding_needed": out.get("semantic_grounding_needed"),
        "question_like": out.get("question_like"),
        "explicit_compute_action": out.get("explicit_compute_action"),
        "explanation_intent": out.get("explanation_intent"),
        "grounding_intent": out.get("grounding_intent"),
        "compute_intent": out.get("compute_intent"),
        "unknown_acronyms": out.get("unknown_acronyms"),
        "confidence": out.get("confidence"),
        "confidence_band": out.get("confidence_band"),
        "reasoning": out.get("reasoning"),
        "provider": out.get("provider"),
        "pipeline_enabled": out.get("pipeline_enabled"),
        "pipeline_fallback_stage": out.get("pipeline_fallback_stage"),
        "pipeline_repair_count": out.get("pipeline_repair_count"),
...
def _heuristic_plan(message: str, payload: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
    ...
    early_follow_up_mode = _safe_str(normalization.get("follow_up_mode"))
    early_selected = list(normalization.get("selected_molecules") or [])
    early_mentioned = list(normalization.get("mentioned_molecules") or [])
    early_batch_request = bool(normalization.get("batch_request")) or len(early_selected) > 1 or len(early_mentioned) > 1
    if early_batch_request:
        early_follow_up_mode = None
    if early_follow_up_mode and not normalization.get("maybe_structure_hint") and not normalization.get("structures"):
        routing = dict(routing)
        routing["query_kind"] = "compute_ready"
        routing["chat_only"] = False
        routing["semantic_grounding_needed"] = False
    ...
    if batch_request or len(mentioned_molecules) >= 2:
        follow_up_mode = None
    clarification_kind = None
    if follow_up_mode and not structure_query and not structures:
        routing = dict(routing)
        routing["query_kind"] = "compute_ready"
        routing["chat_only"] = False
        routing["semantic_grounding_needed"] = False
    ...
    if batch_request and not selected_molecules and len(mentioned_molecules) >= 2:
        selected_molecules = [str(item).strip() for item in (normalization.get("canonical_candidates") or []) if _safe_str(item)]
        if selected_molecules:
            target_scope = target_scope or "all_mentioned"
            selection_mode = selection_mode or "implicit_all"
            inferred_batch_size = len(selected_molecules)
    missing_slots: List[str] = []
    needs_clarification = False

    if job_type != "resolve_structure" and not structure_query and not structures and not selected_molecules:
...
def _coerce_plan_to_dict(plan_obj: Any) -> Dict[str, Any]:
    ...
    for key in (
        ...
        "query_kind", "planner_lane", "semantic_grounding_needed", "question_like",
        "lane_locked", "locked_lane", "reasoning",
        "explicit_compute_action", "explanation_intent", "grounding_intent",
        "compute_intent", "unknown_acronyms", "chat_response",
        ...
        "mentioned_molecules", "target_scope", "selection_mode", "selection_hint",
        "selected_molecules", "analysis_bundle", "batch_request", "batch_size",
...
def _safe_plan_message(message: str, payload: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _enrich_plan(plan_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        planner_authoritative = bool(_safe_str(enriched.get("planner_lane")) or bool(enriched.get("lane_locked")))
        for key in (
            "question_like",
            ...
            "follow_up_job_type",
            "follow_up_orbital",
            "follow_up_basis_hint",
            "follow_up_method_hint",
            ...
            "pipeline_repair_count",
        ):
            if _value_missing(enriched.get(key)) and message_normalization.get(key) not in (None, [], ""):
        ...
        normalization_force_compute = bool(
            ...
            or (
                normalization_query_kind == "compute_ready"
                and not normalization_force_grounding
                and (
                    bool(message_normalization.get("explicit_compute_action"))
                    or bool(list(message_normalization.get("analysis_bundle") or []))
                )
                and (
                    bool(message_normalization.get("structures"))
                    or bool(_safe_str(message_normalization.get("maybe_structure_hint")))
                    or bool(list(message_normalization.get("canonical_candidates") or []))
                )
            )
        )

        if not planner_authoritative:
            for key in ("query_kind", "planner_lane", "chat_only", "semantic_grounding_needed"):
                if _value_missing(enriched.get(key)) and message_normalization.get(key) not in (None, [], ""):
                    enriched[key] = message_normalization.get(key)
        if _value_missing(enriched.get("clarification_kind")) and message_normalization.get("follow_up_requires_context"):
            enriched["clarification_kind"] = "continuation_targeting"
        plan_query_kind = _safe_str(enriched.get("planner_lane") or enriched.get("query_kind"))
        if not plan_query_kind and not planner_authoritative:
            plan_query_kind = normalization_query_kind

        if normalization_follow_up:
            enriched["follow_up_mode"] = normalization_follow_up
            plan_query_kind = "compute_ready"
            enriched["semantic_grounding_needed"] = False
        elif normalization_force_grounding:
            plan_query_kind = normalization_query_kind or "grounding_required"
            enriched["semantic_grounding_needed"] = True
        elif normalization_force_compute:
            plan_query_kind = "compute_ready"
            enriched["semantic_grounding_needed"] = False

        normalized_follow_up_job_type = _normalize_job_type(
            message_normalization.get("follow_up_job_type"),
            message_normalization.get("follow_up_job_type"),
        )
        normalized_task_job_type = _normalize_job_type(
            message_normalization.get("maybe_task_hint"),
            message_normalization.get("maybe_task_hint"),
        )
        if plan_query_kind != "chat_only":
            if _safe_str(enriched.get("intent")) == "chat":
                enriched["intent"] = "analyze"
            if _safe_str(enriched.get("job_type")) in {"", "chat", "analyze"}:
                computed_job_type = normalized_follow_up_job_type or normalized_task_job_type
                if not computed_job_type:
                    analysis_bundle = {str(item).upper() for item in list(message_normalization.get("analysis_bundle") or []) if _safe_str(item)}
                    if {"HOMO", "LUMO"} & analysis_bundle:
                        computed_job_type = "orbital_preview"
                    elif "ESP" in analysis_bundle:
                        computed_job_type = "esp_map"
                    elif "CHARGE" in analysis_bundle:
                        computed_job_type = "partial_charges"
                if computed_job_type:
                    enriched["job_type"] = computed_job_type
            if not _safe_str(enriched.get("orbital")) and message_normalization.get("follow_up_orbital"):
                enriched["orbital"] = message_normalization.get("follow_up_orbital")

        enriched["query_kind"] = plan_query_kind or enriched.get("query_kind")
        enriched["planner_lane"] = _safe_str(enriched.get("planner_lane")) or plan_query_kind or None
        enriched["chat_only"] = bool(plan_query_kind == "chat_only")
        enriched["semantic_grounding_needed"] = bool(
            enriched.get("semantic_grounding_needed") or plan_query_kind == "grounding_required"
        )
        ...
        if plan_query_kind == "grounding_required":
            raw_variants = {
        ...
        enriched["lane_locked"] = bool(enriched.get("lane_locked") or bool(plan_query_kind))
        enriched["locked_lane"] = _safe_str(enriched.get("locked_lane")) or plan_query_kind or None
...
def _annotation_hints_chat_only(normalization: Mapping[str, Any]) -> bool:
    return bool(
        normalization.get("question_like")
        and not normalization.get("explicit_compute_action")
        and not list(normalization.get("analysis_bundle") or [])
        and not normalization.get("follow_up_mode")
    )


def _annotation_hints_grounding_required(normalization: Mapping[str, Any]) -> bool:
    if normalization.get("semantic_descriptor"):
        return True
    unknown_acronyms = list(normalization.get("unknown_acronyms") or [])
    if not unknown_acronyms:
        return False
    return bool(normalization.get("explicit_compute_action") or normalization.get("question_like"))
...
def _explicit_structure_from_text(text: str) -> Optional[str]:
    ...
    if _annotation_hints_chat_only(normalization):
        return None
    if _annotation_hints_grounding_required(normalization):
        return None
    ...
    follow_up_safe_only = bool(normalization.get("follow_up_requires_context") and not normalization.get("structures"))
    if follow_up_safe_only:
        normalized_candidates = [
            token
            for token in normalized_candidates
            if _looks_like_plain_molecule_name(token) or _is_formula_like(token)
        ]
        if not normalized_candidates and not _safe_str(normalization.get("maybe_structure_hint")):
            return None
    hint = _safe_str(normalization.get("maybe_structure_hint") or structure_hypotheses.get("primary_candidate"))
    if follow_up_safe_only and hint and not (_looks_like_plain_molecule_name(hint) or _is_formula_like(hint)):
        hint = ""
    ...
    for candidate in list(analysis.get("canonical_candidates") or []):
        token = _safe_str(candidate)
        if follow_up_safe_only and not (_looks_like_plain_molecule_name(token) or _is_formula_like(token)):
            continue
        if token and not _is_follow_up_placeholder_query(token):
            return token
    fallback = extract_structure_candidate(raw)
    if fallback and not _is_follow_up_placeholder_query(fallback):
        if follow_up_safe_only and not (_looks_like_plain_molecule_name(fallback) or _is_formula_like(fallback)):
            return None
...
def _is_follow_up_placeholder_query(query: str) -> bool:
    ...
    if len(analysis_hits) >= 2:
        return True
    normalized_token = _normalize_text_token(token)
    if analysis_hits and normalized_token:
        residual = re.sub(
            r"\b(homo|lumo|esp|orbital|charge|charges|basis|optimize|optimization|"
            r"same|this|that|it|again|more|also|too|go)\b",
            " ",
            normalized_token,
            flags=re.IGNORECASE,
        )
...
def _apply_session_continuation(out: Dict[str, Any], *, source_text: str = "") -> Dict[str, Any]:
    ...
    if not follow_up_mode and not authoritative_action_plan:
        ...
        if strong_follow_up and not explicit_from_text and not batch_selected:
...
def _merge_plan_into_payload(
    ...
    *,
    ...
    for key in (
        ...
        if _value_missing(out.get(key)) and plan.get(key) not in (None, [], ""):
            out[key] = plan.get(key)
...
def _prepare_payload(payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    allow_explicit_structure_fallback = bool(
        explicit_candidate
        and (
            _looks_like_locant_structure_name(explicit_candidate)
            or (
                float(raw_normalization.get("structure_confidence") or 0.0) >= 0.90
                and (
                    explicit_candidate.lower() == _safe_str(raw_normalization.get("maybe_structure_hint")).lower()
                    or explicit_candidate.lower()
                    in {
                        str(item).strip().lower()
                        for item in list(raw_normalization.get("candidate_queries") or [])
                        if str(item).strip()
                    }
                )
            )
    ...
    if (
        (not planner_requires_structure_clarification or allow_explicit_structure_fallback)
        ...
        if candidate and len(candidate.strip()) >= 2:
            ...
            _preserve_structure_decomposition(data, source_text=raw_message)
            if _looks_like_locant_structure_name(candidate):
                data["structure_locked"] = True
    ...
    if data["job_type"] not in {"resolve_structure"}:
        try:
            decision = execution_guard_from_payload(data)
        except ExecutionGuardViolation:
            raise HTTPException(
                status_code=400,
                detail=(
                    "援ъ“瑜??몄떇?????놁뒿?덈떎. 遺꾩옄 ?대쫫, XYZ 醫뚰몴 ?먮뒗 atom-spec???쒓났??二쇱꽭?? / "
                    "Structure not recognized. Please provide a molecule name, XYZ coordinates, or atom-spec text."
                ),
            )
        if decision.action != "compute":
            raise HTTPException(
                status_code=400,
                detail=(
                    "怨꾩궛???좊즺?섍린 ??援ъ“媛??뺤젙?섏? ?딆븯?꾩뒿?덈떎. / "
                    "The request is not compute-ready because the structure has not been locked."
                ),
            )
