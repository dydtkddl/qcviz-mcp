...
def _advisor_functional_to_runner_method(
    functional: Optional[str],
    preset_data: Optional[Dict[str, Any]] = None,
) -> Optional[str]:
    """Convert advisor functional labels into compute-runner-safe method names."""
    xc = None
    if isinstance(preset_data, dict):
        pyscf_settings = preset_data.get("pyscf_settings") or {}
        if isinstance(pyscf_settings, dict):
            xc = pyscf_settings.get("xc")

    candidate = str(xc or functional or "").strip()
    if not candidate:
        return None

    normalized = candidate.lower()
    if len(normalized) > 1 and normalized[0] in {"u", "r"} and normalized[1].isalpha():
    ...
    return _ADVISOR_XC_TO_RUNNER_METHOD.get(normalized, candidate)
...
def apply_preset_to_runner_kwargs(
    ...
    preset_data = _extract_preset_data(advisor_plan.get("preset"))
    functional = _advisor_functional_to_runner_method(
        advisor_plan.get("applied_functional"),
        preset_data,
    )
