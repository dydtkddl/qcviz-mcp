...
_CITATIONS = {
    ...
    "wb97xd": {
        ...
        "short": (
            "Chai, Head-Gordon, Phys. Chem. Chem. Phys. 2008, 10, 6615"
        ),
        "bibtex": (
            "@article{Chai2008,\n"
            "  author  = {Chai, Jeng-Da and Head-Gordon, Martin},\n"
            "  title   = {Long-range corrected hybrid density functionals "
            "with damped atom-atom dispersion corrections},\n"
            "  journal = {Phys. Chem. Chem. Phys.},\n"
            "  volume  = {10},\n"
            "  pages   = {6615--6620},\n"
            "  year    = {2008},\n"
...
class MethodsSectionDrafter:
    ...
    def _draft_method_paragraph(
        ...
        func_lower = (
            rec.functional.lower()
            .replace("-", "")
            .replace("(", "")
            .replace(")", "")
        )
        if len(func_lower) > 1 and func_lower[0] in {"u", "r"} and func_lower[1].isalpha():
            func_lower = func_lower[1:]
        func_cite_key = None
        sorted_keys = sorted(
