...
def main() -> int:
    ...
    header = (
        f"{'Molecule':<20} {'Basis':<10} {'E(Hartree)':<14} "
    ...
    for mol in MOLECULES:
        for basis in mol.basis_sets:
            ...
            try:
                ...
                print(
                    f"{mol.name:<20} {basis:<10} {scf_res.energy_hartree:<14.6f} "
                ...
                elapsed = time.time() - t0
                failed += 1
                print(f"{mol.name:<20} {basis:<10} ERROR: {e}")
                results.append({
                    "name": mol.name,
                    "basis": basis,
                    "status": "error",
    ...
    print("=" * len(header))
    total = passed + failed
    print(f"Total: {total} runs ({len(MOLECULES)} molecules × basis sets), {passed} passed, {failed} failed")
